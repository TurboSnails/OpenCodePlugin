...
async function runCheck(name, cfg, dir, run) {
    ...
    try {
        ...
        return {
            ok: false,
            detail: `${name} health check failed: ${message}. ` +
    ...
    return {
        ok: false,
        detail: `${name} ran but created no files in an isolated directory. ` +
            `The delegate's permission/sandbox flags in cli-dispatch.config.json may be read-only.`,
    };
}
export function makeCheckTool(name, cfg, run = runDelegate) {
    return tool({
