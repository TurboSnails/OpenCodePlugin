...
export function buildChangeSummary(before, after, cwd) {
    if (before === after)
        return null;
    let stat = "";
    try {
        ...
        if (result.success)
            stat = result.stdout.toString().trimEnd();
    }
    catch { }
    const beforeUntracked = new Set(before.split("\n").filter((l) => l.startsWith("?? ")).map((l) => l.slice(3)));
...
//# sourceMappingURL=worktree-summary.js.map