...
function checkBinaries(config, ctx) {
...
function checkAuthenticated(config, ctx) {
    const problems = [];
    const binaries = new Set(Object.values(config.delegates).map((d) => d.binary));
    for (const b of binaries) {
        if (b === "claude") {
            const okFile = existsSync(join(ctx.homeDir, ".claude", ".credentials.json")) ||
                existsSync(join(ctx.homeDir, ".claude.json"));
            if (!okFile)
                problems.push("claude is not authenticated");
        }
        else if (b === "codex") {
            if (!existsSync(join(ctx.homeDir, ".codex", "auth.json")))
                problems.push("codex is not authenticated");
        }
    }
...
async function checkWritability(config, run) {
    const failures = [];
...
function globalCommandsDir(ctx) {
    return join(ctx.homeDir, ".config", "opencode", "commands");
}
function checkSlashCommands(config, ctx) {
