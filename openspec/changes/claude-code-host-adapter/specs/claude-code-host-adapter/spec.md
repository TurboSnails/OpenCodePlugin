...
Once a delegation to `codex` or `opencode` has been started, the system SHALL route subsequent user messages in that Claude Code conversation to the same delegate's session (continuing its external session id), without requiring the user to repeat the delegate-start command, until a different delegate command is issued or the user returns to Claude Code.
