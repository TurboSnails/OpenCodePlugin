# delegate-health-check Specification

## Purpose

On-demand verification that a configured delegate CLI can actually write files when spawned with its configured arguments, so permission/sandbox config regressions are caught deliberately rather than discovered by users at runtime.
